...
void loop() {
  ...
  if (((now - lastMsg) > Sleep_time)||(firstRun==1)) {
    ...
    client.publish("/Solar1/Umiditate_sol", String(moisture_percentage).c_str());
    client.publish("/Solar1/Temperatura_aer", String(temp_air).c_str());
    client.publish("/Solar1/Umiditate_aer", String(humi_air).c_str());
  }
}
